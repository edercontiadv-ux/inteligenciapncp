import { FileText, BookOpen, Building2, MapPin, Calendar, ExternalLink } from 'lucide-react';
...
export default function CardItem({ data, index, onSelect, isSelected }: CardItemProps) {
    ...
    return (
      <div
        className={`group relative rounded-xl border transition-all duration-300 cursor-pointer
          ${isSelected
            ? 'border-brand-gold/50 bg-brand-gold/[0.03] shadow-md shadow-brand-gold/5'
            : 'border-brand-sand/30 bg-white hover:border-brand-sand/60 hover:shadow-lg hover:shadow-brand-navy/5'
          }`}
        onClick={() => onSelect(id)}
      >
        <div className={`h-1 rounded-t-xl transition-colors duration-300 ${isSelected ? 'bg-brand-gold' : 'bg-brand-navy/10 group-hover:bg-brand-navy/20'}`} />
        <div className="p-5">
          <div className="flex justify-between items-start mb-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                <span className="font-body text-xs font-semibold text-brand-navy uppercase tracking-wide">
                  Contrato
                </span>
                <span className="font-body text-xs text-brand-navy/40 block">
                  {data.numeroContrato}/{data.anoContrato}
                </span>
              </div>
            </div>
            <div
              className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all duration-200 shrink-0
                ${isSelected ? 'border-brand-gold bg-brand-gold' : 'border-brand-sand/50 group-hover:border-brand-navy/30'}`}
          ...
          <p className="font-body text-sm text-brand-ink/80 mb-4 line-clamp-3 leading-relaxed" title={data.objetoContrato}>
            {data.objetoContrato}
          </p>
          ...
          <div className="mt-4 pt-4 border-t border-brand-sand/20 flex justify-between items-center">
            <span className="font-heading text-base text-brand-navy">
              {data.valorInicial ? data.valorInicial.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }) : 'Valor não informado'}
            </span>
            <a href={data.linkArquivo} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="inline-flex items-center gap-1 text-xs font-medium text-brand-navy/50 hover:text-brand-navy transition-colors">
    ...
    >
      ...
      <div className="p-5">
        <div className="flex justify-between items-start mb-3">
          ...
          <div
            className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all duration-200 shrink-0
              ${isSelected ? 'border-brand-forest bg-brand-forest' : 'border-brand-sand/50 group-hover:border-brand-navy/30'}`}
            onClick={(e) => e.stopPropagation()}
          >
            {isSelected && (
              <svg className="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
