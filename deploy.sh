...
gcloud run deploy $SERVICE_NAME \
  ...
  --set-env-vars "NODE_ENV=production,DATABASE_URL=$DATABASE_URL,JWT_SECRET=$JWT_SECRET,NEXT_PUBLIC_SITE_URL=$NEXT_PUBLIC_SITE_URL,SENTRY_DSN=$SENTRY_DSN,RESEND_API_KEY=$RESEND_API_KEY,EMAIL_FROM=$EMAIL_FROM" \
  --memory 512Mi \
  --timeout 300
